...
function Cartoons() {
  ...
  const [listcartoons, setListcartoons] = useState([]);
  ...
  const fetchCartoons = async (page) => {
    setLoading(true);
    try {
      const listcartoons = await axios.get(
        `https://phim.nguonc.com/api/films/danh-sach/hoat-hinh?page=${page}`
      );

      if (listcartoons.data.status === "success") {
        // const intersection = array1.filter(obj1 =>
        //   array2.some(obj2 => obj1.id === obj2.id)
        // );
        setListcartoons((prevMovies) => [
          ...prevMovies,
          ...listcartoons.data.items,
        ]);
        setHasMore(page < listcartoons.data.paginate.total_page);
      }
    } catch (error) {
