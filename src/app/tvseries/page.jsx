...
function Tvseries() {
  ...
  const [listtvseries, setListtvseries] = useState([]);
  ...
  const fetchTvseries = async (page) => {
    setLoading(true);
    try {
      const listtvseries = await axios.get(
        `https://phim.nguonc.com/api/films/danh-sach/phim-bo?page=${page}`
      );

      if (listtvseries.data.status === "success") {
        // const intersection = array1.filter(obj1 =>
        //   array2.some(obj2 => obj1.id === obj2.id)
        // );
        setListtvseries((prevMovies) => [
          ...prevMovies,
          ...listtvseries.data.items,
        ]);
        setHasMore(page < listtvseries.data.paginate.total_page);
      }
    } catch (error) {
